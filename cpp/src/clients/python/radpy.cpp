...
//static PyObject* radia_ObjGeometry(PyObject *self, PyObject *args) 
{
	...
	try
	{
		...
		//  but then this is a malloc error...
		 ...
		 // NOTE: seem to have experienced crashes without this
	}
	catch(const char* erText)
	{
		PyErr_SetString(PyExc_RuntimeError, erText);
...
static PyObject* radia_MatSatLamFrm(PyObject* self, PyObject* args)
{
	...
	try
	{
		...
		if(o3 != 0)
		{
			if(PyNumber_Check(o3)) p = PyFloat_AsDouble(o3);
			else
			{
				if(p == 0)
				{
		...
		if(o4 != 0)
		{
			if(p == 0)
			{
				if(PyNumber_Check(o4)) p = PyFloat_AsDouble(o4);
			}
			else
			{
 ...
 ***************************************************************************/
static PyObject* radia_Fld(PyObject* self, PyObject* args, PyObject* kwargs)
{
...
static PyObject* radia_FldFocPot(PyObject* self, PyObject* args)
{
	...
	{
		int indObj=0, np=0;
		if(!PyArg_ParseTuple(args, "iOOi:FldFocPot", &indObj, &oP1, &oP2, &np))
...
/************************************************************************//**
 * Reports which backend (GPU or CPU) serviced the most recent RlxPre
 * interaction-matrix assembly
 ***************************************************************************/
static PyObject* radia_UtiAsmLastBackend(PyObject* self, PyObject* args)
{
	PyObject* oRes = 0;
	try
	{
		int backend = -1;
		g_pyParse.ProcRes(RadUtiAsmLastBackend(&backend));
